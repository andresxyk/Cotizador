...
public class SucursalController {
	...
	final static Logger log = LogManager.getLogger(SucursalController.class);

	@RequestMapping(value = "/search-sucursal", method = { RequestMethod.POST }, consumes = {
			MediaType.APPLICATION_JSON_VALUE }, produces = { MediaType.APPLICATION_JSON_VALUE })
	@Operation(description = "Método para buscar sucursal en base al filtro proporcionado en la petición.", tags = {
			"Cotización" })
				...
				if(request.validarMarca(request)) {
					if(request.validarFiltroSucursal(request)) {
						...
						request.getGDA_menssage()
								.setDescripcion("Los campos ssucursal y csucursal son vacios, no se puede validar");
