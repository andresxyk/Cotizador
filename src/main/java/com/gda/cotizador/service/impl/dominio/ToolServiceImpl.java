...
import com.gda.cotizador.dto.cotizacion.TOrdenSucursalDto;
...
public class ToolServiceImpl implements ToolDominio{
			...
			for (Coding coding : request.getCode().getCoding()) {
				...
				if(listExamenesConfigDto.size()>0) {
					...
					if(listExamenesConfigDto.get(0).getBrequierecita()){
						...
						//Si existe el Filtro de Distancia y codigo postal regresa las sucursales Filtradas
						if( request.getFiltro() != null && request.getFiltro().getCodigopostal() != null &&
								request.getFiltro().getCodigopostal() != 0 && request.getFiltro().getDistancia() != null){
							coding.setSucursalesProcesa(filterByDistanceAndZipCode(request, consultasCotizacionDao.getListSucursalesProcesa(listExamenesConfigDto.get(0).getCexamen())));
						}else{
	 ...
	 * @return retorna la lista de las sucursales cuya distancia entre el codigo postal del filtro de de la sucursal sea menor a la distancia del filtro
	 */
	private String[] filterByDistanceAndZipCode(RequestCotizacionDto request, String[] listaIdSucursales) {
		List<SucursalDto> sucuralesByMarca = consultasDao.getListSearchSucursalDto(request.getFiltro(), request.getHeader().getMarca());
		...
		for (int i = 0; i < sucursales.size(); i++) {
			Double distance = getDistanceBetweenTwoPoints(sucursales.get(i).getCodigopostal(), request.getFiltro().getCodigopostal());
